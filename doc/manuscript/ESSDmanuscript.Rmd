...
n_relevant_variables = nrow(C_variables) - 8 + 2  # n variables relevant = rows in C_variables - headers and total + 2 that are grouped with similar variables
...
flux_variables_with_records <- c("NPP_woody", "ANPP_woody", "ANPP_woody_ag", "BNPP_root_fine","BNPP_root.turnover_fine","BNPP_root_coarse" )
...
For the latter, we used an R script to automatically harvest (scrape) the URL, citation, abstract and language of the publications, based on their DOI, using R package `rvest`[ @rvest]). That information was manually retrieved when the web scraping failed. 
...
ForC contained records for `r n_relevant_variables_with_data` of the `r n_relevant_variables` variables relevant to EFDB (Table `r table_variables`, Fig. `r fig_variable_mapping`).
...
Relevant flux variable records (n=`r n_ind_records_flux`) were limited to the biomass pools (aboveground, belowground, or total) and together constituted `r round(n_ind_records_flux/n_ind_records_EFDB_variables,2)*100`% of ForC's independent records relevant to EFDB.
...
<!-- this paragraph will need to be revisited after we solve issues 42 and 43-->
...
In terms of variables transferred, ...
*[DETAILS]*
...
Moreover, the process of selecting studies to review for transfer has yet to be optimized for representation across geographic regions, forest types, and variables, with the result that records transferred to date are unevenly distributed across these categories (Figs. `r fig_map`-`r fig_histograms`, Table `r table_variables`).
Future work reviewing records for transfer will be most valuable if priority is given to those from currently under-represented regions and forest types, *to records from countries relying on existing records (as opposed to regular collection of new data through forest inventories)*, to the variables most needed by EFDB users, and to more contemporary records.

Moreover, there are innumerous published EFDB-relevant forest C data that are not included in ForC, with more being published on a nearly daily basis. 
...
Such reviews are necessary to assess the availability of published EFDB-relevant forest C records, versus those that require additional data collection and analysis. 
