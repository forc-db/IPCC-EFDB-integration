...
While IPCC guidance specifies that all living vegetation should be included in biomass estimates, forest censuses and biomass estimates do not consistently include life forms other than dicot trees (e.g., lianas, ferns, palms, bamboo), although these do tend to be censused when they constitute a large proportion of the biomass [@refs_from_ForC].
...
Filling knowledge gaps in other EFDB-relevant variables will require more effort, but this effort is warranted given their importance for forest C accounting.
...
We recommend that, in order to make research valuable to C accounting under IPCC guidelines, researchers calculate and report results according to IPCC standards (Table `r table_recommendations`).
...
The ForC database contains large numbers of records that could potentially be useful for C accounting under IPCC guidelines.
Here we have developed a framework for transferring these records to the EFDB, making them more accessible for forest C accounting under IPCC guidelines [@ipcc_2006_2006;@ipcc_2019_2019].
...
The records transferred to EFDB and present in ForC are very unevenly distributed across variables, regions, and forest types (Figs. `r fig_map`-`r fig_histograms`, Table `r table_variables`), reflecting broader patterns in allocation of research effort.

Going forward, forest researchers can make their research more useful for forest C accounting under IPCC guidelines by calculating and reporting results in ways that are compatible with IPCC guidelines (Tables `r table_pools`, `r table_recommendations`).
In addition, substantial investments in research and researchers focused on ground-based measurement of forest C will be required to fill knowledge gaps and thereby increase the accuracy of forest C accounting. 
...
We view this publication as one step towards that goal.
