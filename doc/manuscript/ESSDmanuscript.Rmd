...
As of `r format(Sys.time(), '%B %d, %Y')`<!--inserts date upon knit-->, we had reviewed and sent `r n_records_sent_to_EFDB` records to EFDB, `r n_records_posted_EFDB` of which have been reviewed, accepted, and posted (Figs. `r fig_map`-`r fig_histograms`, Table `r table_variables`).
*[DETAILS]*
...
So far, only ~`r round(n_records_sent_to_EFDB/n_records_EFDB_variables,2)*100`% of the EFDB-relevant data in ForC have been sent to EFDB.
...
*Other EFDB-relevant variables require more effort but are warrented given their importance for forest C accounting.*
...
We recommend that, unless they have some specific reason to do otherwise, researchers calculate and report the values according to IPCC standards (`r table_recommendations`).
