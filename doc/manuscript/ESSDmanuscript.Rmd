...
With input from the IPCC's Technical Support Unit, we reviewed the list of ForC variables to identify those that were relevant to EFDB and to appropriately map them into the C pools and stock, stock changes, increments, and losses (Fig. `r fig_variable_mapping`).
As described in section `r section_new_variables`, we also defined `r n_new_variables` new EFDB-relevant variables that were not previously represented. 
