...
Going into the future, the fate of this important CO~2~ sink is highly uncertain, depending both upon forest responses to climate change, which are likely to reduce the sink strength [@refs], and on human conservation, restoration, and management of forests [@refs].
...
Yet, envisioned forest-based climate change mitigation initiatives do not always correspond to actual emission reductions through on-the-ground implementation [e.g., @badgley_systematic_2022]<!--could be good to add some more references here-->.
...
Moreover, high variability of forest C cycling within ecozones [e.g., @cook-patton_mapping_2020; @refs] implies that it is useful for practitioners to have access to locally-specific information, when available.
...
Thus, C cycle variables relevant to the IPCC methodology and to EFDB include C stocks, increments, and fluxes in the IPCC-defined pools (Fig. `r fig_variable_mapping`).
...
IPCC's guidance is that the understory may be excluded the understory if it constitutes a "minor" component, *where quantitative definitions of "understory" and "minor" is not provided*<!--verify-->, but where a commonly applied minimum size sampling threshold for mature forests would be 10 cm stem diameter at breast height (DBH).
...
Not required for forest land remaining forest land under Tier 1 methodology [@ipcc_2006_2006].
...
#this is for a two-column figure. for one-column figure, use {r, out.width = "8.3cm", echo = FALSE, fig.cap = "one column figure"}
knitr::include_graphics("figures_tables/PLACEHOLDER_World_Map_of_sites.png")
```
...
#this is for a two-column figure. for one-column figure, use {r, out.width = "8.3cm", echo = FALSE, fig.cap = "one column figure"}
knitr::include_graphics("figures_tables/PLACEHOLDER_Williams_Histogram_of_dominant_vegetation.png")
```
