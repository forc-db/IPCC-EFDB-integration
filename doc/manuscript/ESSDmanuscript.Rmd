...
n_records_sent_fluxes = n_records_sent_to_EFDB-n_records_sent_stocks-n_records_sent_increments
...
In terms of variables transferred, records were sent for `r n_relevant_variables_records_sent` variables, including variables from each C pool (Table `r table_variables`).
...
`r round(n_records_sent_litter/n_records_sent_to_EFDB,2)*100`% for litter (OL layer or entire O horizon),
and `r round(n_records_sent_soil/n_records_sent_to_EFDB,2)*100`% for SOM/ SOC.
Increment records totaled `r round(n_records_sent_increments/n_records_sent_to_EFDB*100, 0)`% of records sent, including
`r round(n_records_sent_delta.agb/n_records_sent_to_EFDB*100,2)`% for aboveground biomass increments 
and `r round(n_records_sent_delta.litter/n_records_sent_to_EFDB*100,2)`% for organic horizon increments.
The remaining (`r round(n_records_sent_fluxes/n_records_sent_to_EFDB*100, 0)`%) of records sent described fluxes,
all of which were either inputs or outputs to the aboveground biomass pool,
and a subset of which also described inputs to the dead wood or litter pool (Table `r table_variables`, Fig. `r fig_variable_mapping`).
...
Such reviews valuable for assessing the availability of published EFDB-relevant forest C records, versus those that require additional data collection and analysis. 
