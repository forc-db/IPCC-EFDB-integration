...
Relative to forested area, submitted data under-represented the tropics and over-represented temperate and boreal biomes (Fig. `r fig_map`).
Submitted data were concentrated in climates that are relatively common over Earth's land area (Fig. `r fig_climate`).
...
![**Figure `r fig_climate`. Distribution of ForC records relevant (dots) and submitted (triangles) to EFDB within the global climate space of mean annual temperature (MAT) and mean annual precipitation (MAP).** Climate data are from *WorldClim 2 (0.5 resolution, Fick & Hijmans, 2017*. Background colors indicate the global land area with each MAT-MAP combination. GitHub issue 61:https://onlinelibrary-wiley-com.smithsonian.idm.oclc.org/doi/full/10.1111/gcb.15353](figures_tables/Climate_of_forC_sites_IPCC.png)
