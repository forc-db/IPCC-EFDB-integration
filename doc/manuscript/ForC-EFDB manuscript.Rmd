...
With input from the IPCC's Technical Support Unit and referencing IPCC guidance [@ipcc_good_2003; @ipcc_2006_2006; @ipcc_2019_2019], we determined how EFDB fields should be populated using ForC fields (summarized in `r table_ForCfieldmapping`).
...
We identified ForC variables that were relevant to the IPCC methodology and to EFDB (`r appendix_IPCC_definitions`).
...
Classification into sub-categories was dependent upon stand age and site history.
...
To submit complete, reviewed ForC records into EFDB, we created R scripts to restructure ForC records and populate EFDB's bulk import form.
...
As of `r format(Sys.time(), '%B %d, %Y')`<!--inserts date upon knit-->, ForC (v4.0) contained `r n_independent_records_ForC` independent records (`r n_records_ForC` total), `r n_ind_records_EFDB_variables` of which were for the `r n_relevant_variables` variables relevant to EFDB (Fig. `r fig_variable_mapping`).
We had reviewed or added `r n_records_reviewed_or_added` records for submission to EFDB, `r n_records_sent_to_EFDB` of which met the criteria for submission, and `r n_records_posted_EFDB` of which had been reviewed by EFDB's review panel, accepted, and posted (Table `r table_variables`).
The `r round((n_records_reviewed_or_added -n_records_sent_to_EFDB )/n_records_reviewed_or_added,2)*100`% attenuation between records we reviewed and those submitted to EFDB was attributable to the presence of digitized records and records where a variable's value had been calculated as the sum or difference of related variables rather than presented directly in the text.
...
ForC v4.0 contained records for `r n_relevant_variables_with_data` of the `r n_relevant_variables` variables (or closely-related variable groups) relevant to EFDB (Fig. `r fig_variable_mapping`, Table `r table_variables`).
Records were submitted for `r n_relevant_variables_records_sent` variables (or closely-related variable groups), including variables from each C pool (Table `r table_variables`).
The records available in ForC v4.0 -- and the subset submitted to EFDB -- were very unevenly distributed across variables (Table `r table_variables`). 
...
Increment records totaled `r round(n_records_sent_increments/n_records_sent_to_EFDB*100, 0)`% of records submitted, all but four of which described aboveground biomass.
The remaining `r round(n_records_sent_fluxes/n_records_sent_to_EFDB*100, 0)`% of records submitted described fluxes, all of which were either inputs or outputs to the aboveground biomass pool, a subset of which also described inputs to the dead wood or litter pool (Fig. `r fig_variable_mapping`,Table `r table_variables`).


ForC v4.0 records and the subset submitted to EFDB were distributed across all forested continents, biomes, and forest types, albeit very unevenly (Figs. `r fig_map`-`r fig_histograms`).
Among the records submitted to EFDB, the largest number came from North America, followed by Asia, South America, and Africa (Fig. `r fig_histograms`c), with the most represented FAO ecozones being boreal coniferous forest, temperate continental forest, and temperate mountain systems, followed by tropical rain forests and moist deciduous forests (Fig. `r fig_histograms`b).
...
The largest number of records came from mature forests (>100 years), followed by young and intermediate-aged stands (Fig. `r fig_histograms`d).
