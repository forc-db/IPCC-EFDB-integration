...
We had reviewed or added `r n_records_reviewed_or_added` records for submission to EFDB, `r n_records_sent_to_EFDB` of which had been submitted, and `r n_records_posted_EFDB` of which had been reviewed by EFDB's review panel, accepted, and posted (`r table_variables`).
...
The discrepancy between the number of records submitted and that posted to EFDB is because some records (`r round((1-(n_records_posted_EFDB/n_records_sent_to_EFDB)),2)*100`%) were deemed not applicable to EFDB by the review panel.
...
As of `r format(Sys.time(), '%B %d, %Y')`, the `r n_records_sent_to_EFDB` ForC records posted in EFDB represented  ~`r round(n_records_sent_to_EFDB/n_ind_records_EFDB_variables,2)*100`% of the EFDB-relevant data in ForC and `r percent_forest_records`% of the total EFDB records for forest land.
...
`r round(n_records_sent_litter/n_records_sent_to_EFDB,2)*100`% for litter (entire O horizon),
