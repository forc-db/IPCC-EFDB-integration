...
climate change, database, forest carbon, greenhouse gas inventory, International Panel on Climate Change (IPCC), natural climate solutions
...
Human interactions with forests have shaped Earth's climate for >12,000 years and will continue to do so as we target net-zero emissions.
Accurately characterizing these climate impacts requires accurate forest carbon data available for national greenhouse gas inventories conducted under the Paris Agreement. 
...
We had reviewed or added `r n_records_reviewed_or_added` records for submission to EFDB, `r n_records_sent_to_EFDB` of which met the criteria for submission, and `r n_records_posted_EFDB` of which had been reviewed by EFDB's review panel, accepted, and posted (`r table_variables`).
...
ForC v4.0 contained records for `r n_relevant_variables_with_data` of the `r n_relevant_variables` variables (or closely-related variable groups) relevant to EFDB (Fig. `r fig_variable_mapping`,  `r table_variables`).
Records were submitted for `r n_relevant_variables_records_sent` variables (or closely-related variable groups), including variables from each C pool (`r table_variables`).
The records available in ForC v4.0 -- and the subset submitted to EFDB -- were very unevenly distributed across variables (`r table_variables`). 
...
The remaining `r round(n_records_sent_fluxes/n_records_sent_to_EFDB*100, 0)`% of records submitted described fluxes, all of which were either inputs or outputs to the aboveground biomass pool, a subset of which also described inputs to the dead wood or litter pool (Fig. `r fig_variable_mapping`,`r table_variables`).
...
While aboveground biomass stocks in particular have received -- and continue to receive -- by far the most research attention [`r table_variables`, @anderson-teixeira_carbon_2021;@dubayah_global_2020; @quegan_european_2019; @nisar_nasaisro_2018], production of an accurate global map of forest C stocks remains an ongoing challenge [@araza_decade_2023]. 
Other pools and variables remain very poorly quantified and highly uncertain for many parts of the world [`r table_variables`,@tifafi_large_2018; @anderson-teixeira_carbon_2021], introducing substantive uncertainties into global forest C budgets [@pan_large_2011;@harris_global_2021].
