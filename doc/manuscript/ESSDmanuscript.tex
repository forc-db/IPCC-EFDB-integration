...
\hline
\textbf{Table} & \textbf{Variable} & \textbf{EFDB field} & \textbf{Use} & \textbf{Required}\\
\hline
Sites &  & NA &  & \\
\hline
Measuremennts &  & NA &  & \\
\hline
 & sd, se, lower95\%CI, upper 95\%CI & NA &  & y\\
\hline
 & mean.in.original.units, original.units & NA &  & y\\
\hline
 & data.location.within.source & NA & confirm that data weren't digitized, facilitate finding data in original publication & y\\
\hline
PFT & description & NA &  & \\
\hline
 &  & NA &  & \\
\hline
Citations & (several fields) & NA &  & \\
\hline
\end{tabu}
\end{table}
