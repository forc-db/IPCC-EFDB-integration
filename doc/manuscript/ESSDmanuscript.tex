...
Although most of these variables lacked records in ForC as of May 12,
...
In terms of variables transferred, records were sent for 21 variables,
including variables from each C pool (Table 2). The majority (82\%) of
records sent were for C stocks, including 3\% for total biomass, 53\%
for aboveground biomass, 4\% for components of aboveground biomass (wood
or foliage), 4\% for root biomass, 2\% for components of root biomass
(coarse or fine roots), 5\% for dead wood, 3\% for components of dead
wood (standing or fallen), 4\% for litter (OL layer or entire O
horizon), and 4\% for SOM/ SOC. Increment records totaled 9\% of records
sent, including 8.55\% for aboveground biomass increments and 0.28\% for
organic horizon increments. The remaining (9\%) of records sent
described fluxes, all of which were either inputs or outputs to the
aboveground biomass pool, and a subset of which also described inputs to
the dead wood or litter pool (Table 2, Fig. 1).
...
\emph{others? (SUSAN)}. Such reviews valuable for assessing the
availability of published EFDB-relevant forest C records, versus those
that require additional data collection and analysis.
